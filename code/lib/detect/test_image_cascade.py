...
def get_roi(label1):
    ...
    for i in xrange(31*42):
        ...
        if label>0:
            ...
            if label == 1:
                ...
                _roi_blob5[len][2] = 1 + 24*y - cfg.BODY_PATCH_HEIGHT/2 + cfg.HEAD_PATCH_RADIUS
                len += 1
            elif label == 2: 
                _roi_blob5[len][1] = 1 + 24*x
                ...
                _roi_blob5[len][1] = 1 + 24*x - cfg.BODY_PATCH_WIDTH/2 + cfg.HEAD_PATCH_RADIUS
                _roi_blob5[len][2] = 1 + 24*y
                len += 1
            elif label == 4: 
                _roi_blob5[len][1] = 1 + 24*x - cfg.BODY_PATCH_WIDTH + cfg.HEAD_PATCH_RADIUS*2
                _roi_blob5[len][2] = 1 + 24*y
                len += 1    
            elif label == 5:
                _roi_blob5[len][1] = 1 + 24*x - cfg.BODY_PATCH_WIDTH + cfg.HEAD_PATCH_RADIUS*2
                ...
                _roi_blob5[len][1] = 1 + 24*x - cfg.BODY_PATCH_WIDTH + cfg.HEAD_PATCH_RADIUS*2
                _roi_blob5[len][2] = 1 + 24*y - cfg.BODY_PATCH_HEIGHT + cfg.HEAD_PATCH_RADIUS*2
                len += 1               
            elif label == 7:
                _roi_blob5[len][1] = 1 + 24*x - cfg.BODY_PATCH_WIDTH/2 + cfg.HEAD_PATCH_RADIUS
                _roi_blob5[len][2] = 1 + 24*y - cfg.BODY_PATCH_HEIGHT + cfg.HEAD_PATCH_RADIUS*2
    ...
    for i in xrange(len):
        _roi_blob5[i][0] = 0
        _roi_blob5[i][3] = _roi_blob5[i][1] + cfg.BODY_PATCH_WIDTH
        _roi_blob5[i][4] = _roi_blob5[i][2] + cfg.BODY_PATCH_HEIGHT
        _roi_blob5[i][1] = max(1, _roi_blob5[i][1])
        _roi_blob5[i][2] = max(1, _roi_blob5[i][2])
        _roi_blob5[i][3] = min(_roi_blob5[i][3], cfg.IMAGE_WIDTH)
        _roi_blob5[i][4] = min(_roi_blob5[i][4], cfg.IMAGE_HEIGHT)
